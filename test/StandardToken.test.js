...
contract('BoraToken', function (accounts) {
  ...
  beforeEach(async function () {
    this.token = await BoraToken.new(supply);
  });
  ...
  describe('lock', function () {
    const usr1 = accounts[1];
    const amount = 10;
    const days = 10;

    it('emits an lock event', async function () {
      const { logs } = await this.token.lock(owner, usr1, amount, days, true, {from: owner});
      ...
      assert.equal(ownerBalance,supply - amount);
    });
  });
});